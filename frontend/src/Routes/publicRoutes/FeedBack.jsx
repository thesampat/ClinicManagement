...
const PatientForm = ({ formData, formError, handleInputChange, handleButtonClick }) => (
    ...
    <div className='bg-primary-50 pb-8 rounded-md pt-4 border-2 border-primary-400 '>
        ...
        <button onClick={handleButtonClick}>Submit Patient Form</button>
      </div>
    </div>
  );

  const FeedbackForm = ({ formData, formError, handleInputChange, handleButtonClick ,isProcessing,setIsPorcessing}) => (
    <div className="p-20">
    <div className='bg-primary-50 pb-8 rounded-md pt-4 border-2 border-primary-400 '>
        <div className="pb-8 rounded-md pt-4">
            <div className="px-6 py-6 rounded-md ">
        ...
        {/*  */}
        <CustomButton onClick={handleButtonClick} isProcessing={isProcessing} label={'Save'} />
      </div>
    </div>
  );
...
const initialFormData = {
    ...
    industry: '',
    email: '',
    comments: [{date:'', content:'', signature:''}, {date:'', content:'', signature:''}, {date:'', content:''}],
    Status: "",
    CaseNo: "",
    Date: "",
};
...
export const FeedBack = () => {
    ...
    useEffect(() => {
        ...
        const fetchPatients = async () => {
          try {
            ...
            setPatientList(response.data); // Assuming the API response is an array of patient objects
          } catch (error) {
      ...
      const handleButtonClick = async () => {
        try {
          ...
          if (currentForm === 'initial') {
            ...
            // Once the patient form is submitted successfully, switch to the feedback form
            // await handleSubmit(formData, navigate, setIsProcessing, 'patient');
            ...
            if (selectedPatientId) {
                // If a patient is selected, associate the feedback with that patient
                console.log(selectedPatientId)
                await updateItem({...formData},{ _id: selectedPatientId} , setIsProcessing);
              } else {
                // Handle error if a patient is not selected
                await handleSubmit(formData, navigate, setIsProcessing);
                setCurrentForm('initial');
            }
            // await handleSubmit(formData, navigate, setIsProcessing, 'feedback');
            // setCurrentForm('initial');
          }
        } catch (error) {
    ...
    const handleSelectPatient = (selectedPatientId) => {
        // Load patient details based on the selected patient
        ...
        if (selectedPatient) {
          ...
          console.log(selectedPatientId,"selection")
          ...
          // Handle error if the selected patient is not found
    ...
    <CustomButton onClick={handleButtonClick} isProcessing={isProcessing} label={currentForm === 'initial' ? 'Add New' : 'Back to Patients'} />
          ...
          {currentForm === 'feedback' && (
            <FeedbackForm
              ...
              setIsPorcessing={setIsProcessing}
            />
          )}
        </>
      );
